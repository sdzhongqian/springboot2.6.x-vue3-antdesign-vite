  ...
  >
    <a-spin :spinning="loading">
      <a-form v-bind="formLayout">
        ...
        <a-form-item v-bind="validateInfos.name" label="地区名称">
          <a-input v-model:value="formRef.name" placeholder="地区名称" />
        </a-form-item>
        <a-form-item v-bind="validateInfos.short_name" label="地区简称">
          <a-input v-model:value="formRef.short_name" placeholder="地区简称" />
        </a-form-item>
  ...
  setup(props) {
    ...
    onMounted(() => {
       if (model != null && model != undefined) {
        ...
        formRef.area_cascade = model?.area_cascade;
    ...
    const rulesRef = reactive({
      id: [{ required: false }],
      name: [
        { required: true, max: 50, message: "请输入地区名称且不多余50个字符" },
      ],
      short_name: [{ max: 50, message: "地区简称不能超过50个字符" }],
