  ...
  <page-header-wrapper>
    <a-card :bordered="false">
          ...
          <a-row :gutter="48">
            ...
            <a-col :md="8" :sm="24">
              <a-form-item label="任务分组">
                <a-input
                  v-model:value="queryParam.job_group"
                  placeholder="任务分组"
                  style="width: 160px"
                />
              </a-form-item>
      ...
      <div class="table-operator">
        <a-button type="primary" v-action:timTask:add @click="handleAdd"
      ...
      <s-table
        ...
        size="default"
        rowKey="key"
        :row-key="(record) => record.job_name"
        :columns="columns"
        :data="loadData"
        :loading="loading"
        ...
        <template v-slot:suffix="{ column, record, index, text }">
          <span v-if="column.dataIndex === 'operation'">
            ...
            <a-button
              ...
              @click="() => handleDel(record.job_name, record.job_group)"
              >删除</a-button
            >
            <a-button
              size="small"
...
export default defineComponent({
  ...
  setup() {
    ...
    //执行删除
    const handleDel = (job_name, job_group) => {
      Modal.confirm({
        title: "确定要删除选择的数据吗?",
        content: "数据删除后不可恢复",
        onOk() {
    ...
    const handleDelClick: MenuProps["onClick"] = (e) => {
      if (e.key == "1") {
        const job_name = selectedRows.value.map((map) => {
          return map.job_name;
        });
        const job_group = selectedRows.value.map((map) => {
          return map.job_group;
        });
        handleDel(job_name.join(","), job_group.join(","));
      }
    };
